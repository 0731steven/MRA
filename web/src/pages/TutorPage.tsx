...
import { Button, Drawer, Input, Popconfirm, Segmented, Spin, Tag, message as toast } from "antd";
...
export default function TutorPage() {
  ...
  async function openSource(id: string) {
    const response = await apiClient.get<QuestionDetail>(`/api/question-bank/questions/${id}`);
  ...
  return <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[280px_1fr]">
    ...
    <section className="flex h-[calc(100dvh-168px)] min-h-[590px] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:h-[calc(100vh-132px)] lg:min-h-[640px]">
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-8">
        ...
        <div className="space-y-6">{messages.map((item, index) => <div key={item.id || index} className={`flex gap-3 ${item.role === "user" ? "justify-end" : "justify-start"}`}>{item.role === "assistant" && <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-teal-700 text-white"><RobotOutlined /></span>}<div className={`max-w-[82%] rounded-2xl px-5 py-4 ${item.role === "user" ? "bg-teal-700 text-white" : "border border-slate-100 bg-slate-50 text-slate-700"}`}>{item.role === "assistant" && !item.content ? <div className="flex items-center gap-2 py-1 text-sm text-slate-400"><Spin size="small" />正在检索题库并组织引导…</div> : <div className="tutor-markdown text-[15px] leading-8"><ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>{item.content}</ReactMarkdown></div>}{item.sources && item.sources.length > 0 && <div className="mt-5 border-t border-slate-200 pt-4"><div className="mb-3 text-[11px] font-bold uppercase tracking-wider text-slate-400">可追溯题目 · 点击查看</div><div className="grid gap-2 sm:grid-cols-2">{item.sources.map(source => <button key={source.ID} onClick={() => openSource(source.ID)} className="rounded-xl border border-slate-200 bg-white p-3 text-left transition hover:border-teal-300 hover:bg-teal-50"><div className="flex items-center justify-between"><span className="text-xs font-extrabold text-teal-700">{source.ID}</span><Tag color="cyan" className="!mr-0">{source.hard_level}</Tag></div><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-500">{source.question}</p></button>)}</div></div>}{item.model && <div className="mt-3 text-[10px] text-slate-300">{item.model}</div>}</div>{item.role === "user" && <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-800 text-white"><UserOutlined /></span>}</div>)}</div>
        <div ref={bottom} />
      </div>
      <footer className="border-t border-slate-100 bg-white p-5"><div className="mx-auto flex max-w-3xl items-end gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-teal-500 focus-within:ring-2 focus-within:ring-teal-100"><Input.TextArea value={input} disabled={loading} onChange={event => setInput(event.target.value)} onPressEnter={event => { if (!event.shiftKey) { event.preventDefault(); send(); } }} autoSize={{ minRows: 1, maxRows: 5 }} bordered={false} placeholder={mode === "answer" ? "继续追问，或输入新的题号与知识点…" : "描述想练习的知识点和难度…"} />{loading ? <Button danger shape="circle" aria-label="停止生成" icon={<StopOutlined />} onClick={() => requestController.current?.abort()} /> : <Button type="primary" shape="circle" icon={<SendOutlined />} onClick={() => send()} />}</div><p className="mt-2 text-center text-[11px] text-slate-300">会话自动保存 · Enter 发送 · Shift + Enter 换行</p></footer>
    </section>
    <Drawer open={!!selected} onClose={() => setSelected(null)} width={680} title={selected ? `${selected.ID} · ${selected.qtype}` : "题目详情"}>{selected && <div className="question-markdown space-y-5"><div className="rounded-2xl border border-slate-100 bg-slate-50 p-5"><ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>{selected.question}</ReactMarkdown></div><div className="flex flex-wrap gap-2">{selected.keypoint?.map(item => <Tag color="cyan" key={item}>{item}</Tag>)}</div>{showAnswer ? <><div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5"><h3 className="mb-3 font-bold">参考答案</h3><ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>{selected.answer || "暂无"}</ReactMarkdown></div><div className="rounded-2xl border border-sky-100 bg-sky-50 p-5"><h3 className="mb-3 font-bold">详细解析</h3><ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>{selected.explanation || "暂无"}</ReactMarkdown></div></> : <Button block size="large" icon={<EyeOutlined />} onClick={() => setShowAnswer(true)}>完成思考后查看答案</Button>}<Button block type="primary" size="large" onClick={() => { setSelected(null); setInput(`请用当前的辅导方式带我完成 ${selected.ID}`); }}>用这道题继续练习</Button></div>}</Drawer>
  </div>;
}
