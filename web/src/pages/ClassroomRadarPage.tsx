...
import { Alert, Button, Empty, Form, Input, InputNumber, Progress, Select, Skeleton, Tag, message } from "antd";
import { CheckCircleOutlined, CopyOutlined, PlusOutlined, RadarChartOutlined, ReloadOutlined, SendOutlined, TeamOutlined, WarningOutlined } from "@ant-design/icons";
import { apiClient } from "@/api/client";
import { useAuth } from "@/contexts/AuthContext";

interface Classroom { id: number; name: string; course_name: string; join_code: string; members: number; assignments: number }
...
interface Assignment { id: number; title: string; kind: string; topic: string; recipient_count: number; completed_count: number; question_ids: string[]; created_at?: string }
...
export default function ClassroomRadarPage() {
  ...
  async function publishDiagnostic(values: { title?: string; topic: string; count: number }) {
    if (!selectedId) return;
    setPublishing(true);
    try {
      await apiClient.post(`/api/classrooms/${selectedId}/assignments`, values);
  ...
  return <div className="mx-auto max-w-[1440px]">
    ...
    {loading ? <div className="mt-6"><Skeleton active paragraph={{ rows: 5 }} /></div> : classrooms.length === 0 ? <section className="mt-6 flex min-h-80 flex-col items-center justify-center border border-slate-200 bg-white px-6 text-center"><TeamOutlined className="text-3xl text-teal-700" /><h2 className="mt-4 text-lg font-bold text-slate-900">先创建第一个班级</h2><p className="mt-2 max-w-lg text-base leading-7 text-slate-600">学生使用班级码加入后，你就可以发布短诊断，并从真实作答证据生成班级认知雷达。</p><Button className="mt-5" type="primary" onClick={() => setShowCreate(true)}>创建班级</Button></section> : <>
      <section className="mt-6 flex flex-wrap items-center gap-4 border-y border-slate-200 bg-white px-5 py-4">
        <Select className="min-w-60" value={selectedId} onChange={setSelectedId} options={classrooms.map(item => ({ value: item.id, label: item.name }))} aria-label="选择班级" />
        {radar && <><span className="text-sm text-slate-500">班级码</span><button onClick={copyCode} className="flex min-h-10 items-center gap-2 rounded-lg bg-slate-950 px-3 font-mono text-sm font-bold tracking-wider text-white"><span>{radar.classroom.join_code}</span><CopyOutlined /></button><span className="text-sm text-slate-500">学生输入此码即可加入</span></>}
      </section>
      ...
      {radarLoading && !radar ? <div className="mt-6"><Skeleton active paragraph={{ rows: 10 }} /></div> : radar && <>
        ...
        <section className="mt-4 grid gap-4 bg-slate-950 px-5 py-5 text-white sm:grid-cols-[1fr_auto] sm:items-center" aria-labelledby="next-action-heading">
          ...
          {radar.summary.members === 0 ? <Button icon={<CopyOutlined />} onClick={copyCode}>复制班级码</Button> : radar.summary.attempts === 0 ? <Button type="primary" onClick={showDiagnosticForm}>填写诊断主题</Button> : <Button type="primary" loading={intervening} icon={<RadarChartOutlined />} onClick={createInterventions}>审核并下发任务</Button>}
        </section>

        <section id="diagnostic-panel" className="mt-6 border-y border-slate-200 bg-white" aria-labelledby="diagnostic-heading">
          <div className="grid gap-5 px-5 py-5 lg:grid-cols-[280px_1fr]"><div><h2 id="diagnostic-heading" className="text-lg font-bold text-slate-900">发布短诊断</h2><p className="mt-1 text-sm leading-6 text-slate-600">建议每次 3—5 题；系统会自动兼顾基础、提升和迁移证据。</p></div><Form layout="inline" onFinish={publishDiagnostic} initialValues={{ count: 5 }} className="gap-y-3"><Form.Item name="topic" label="知识点" rules={[{ required: true, message: "请输入知识点" }]}><Input placeholder="如：贝叶斯公式" maxLength={160} /></Form.Item><Form.Item name="title" label="任务名称"><Input placeholder="留空则自动命名" maxLength={180} /></Form.Item><Form.Item name="count" label="题数"><InputNumber min={1} max={8} /></Form.Item><Form.Item><Button type="primary" htmlType="submit" loading={publishing} icon={<SendOutlined />}>发布给全班</Button></Form.Item></Form></div>
        </section>
        ...
        <section className="mt-8 overflow-hidden border border-slate-200 bg-white" aria-labelledby="student-heading">
          ...
          {radar.students.length === 0 ? <Empty className="!my-10" image={Empty.PRESENTED_IMAGE_SIMPLE} description="班级还没有学生" /> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] border-collapse text-left text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="sticky left-0 z-10 bg-slate-50 px-5 py-3 font-semibold">学生</th><th className="px-4 py-3 font-semibold">当前分组</th><th className="px-4 py-3 font-semibold">下一焦点</th><th className="px-4 py-3 font-semibold">掌握度</th><th className="px-4 py-3 font-semibold">证据</th><th className="px-4 py-3 font-semibold">风险</th><th className="px-5 py-3 font-semibold">独立迁移</th></tr></thead><tbody className="divide-y divide-slate-100">{radar.students.map(student => <tr key={student.id} className="group hover:bg-slate-50"><td className="sticky left-0 bg-white px-5 py-3.5 font-semibold text-slate-900 group-hover:bg-slate-50">{student.name}</td><td className="px-4 py-3.5"><Tag color={student.group_label === "迁移挑战组" ? "green" : student.group_label === "证据积累组" ? "blue" : "orange"}>{student.group_label}</Tag></td><td className="px-4 py-3.5 text-slate-700">{student.group_focus || student.next_focus}</td><td className="px-4 py-3.5 font-semibold tabular-nums text-slate-800">{student.overall_mastery}%</td><td className="px-4 py-3.5"><span className="text-slate-700">{evidenceLabel[student.evidence_level]}</span><span className="ml-1 text-slate-600">· {student.questions}题</span></td><td className="px-4 py-3.5 text-slate-700">{student.risk_keypoints ? `${student.risk_keypoints}个知识点` : student.top_error || "暂无"}</td><td className="px-5 py-3.5">{student.independent_transfer ? <span className="font-semibold text-emerald-700"><CheckCircleOutlined className="mr-1" />已验证</span> : <span className="text-slate-600">待验证</span>}</td></tr>)}</tbody></table></div>}
        </section>

        <section className="mt-8 overflow-hidden border border-slate-200 bg-white" aria-labelledby="assignment-heading"><div className="border-b border-slate-200 px-5 py-4"><h2 id="assignment-heading" className="text-lg font-bold text-slate-900">最近任务</h2></div>{radar.assignments.length === 0 ? <Empty className="!my-8" image={Empty.PRESENTED_IMAGE_SIMPLE} description="还没有发布任务" /> : <div className="divide-y divide-slate-100">{radar.assignments.map(item => <div key={item.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-slate-900">{item.title}</span><Tag>{kindLabel[item.kind] || item.kind}</Tag></div><p className="mt-1 text-sm text-slate-600">{item.question_ids.length} 道题 · {item.topic}</p></div><span className="text-sm text-slate-600">{item.completed_count}/{item.recipient_count} 人完成</span><Progress className="!mb-0 w-36" percent={item.recipient_count ? Math.round(item.completed_count / item.recipient_count * 100) : 0} size="small" showInfo={false} /></div>)}</div>}</section>
      </>}
    </>}
  </div>;
}
